...
void MainWindow::savedatabase(QString name, std::vector<uchar> buffer, std::vector<matrix<float,0,1>> face_descriptors)
{
    ...
    QByteArray byteArray1 = matrixToByteArray(face_descriptors);
    ...
    query.bindValue(":matrix", byteArray1);
...
QByteArray MainWindow::matrixToByteArray(const std::vector<dlib::matrix<float,0,1>>& face_descriptors)
{   
    QByteArray byteArray;
    QDataStream stream(&byteArray, QIODevice::WriteOnly);

    //将数据序列化到QByteArray
    stream << static_cast<quint32>(face_descriptors.size()); //写入向量大小
    for(const auto& matrix : face_descriptors)
    {
        const auto& vector = matrixTovector(matrix);
        stream << static_cast<quint32>(vector.size()); //写入矩阵大小
        for(const auto& element : vector)
        {
            stream << element;  // 依次写入矩阵元素
        }
    }
    return byteArray;
}
