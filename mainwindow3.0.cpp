...
m_timer(new QTimer)
{
    ...
    db.setDatabaseName("img1.db");
    db.open();
    query = new QSqlQuery;
    query->exec("CREATE TABLE IF NOT EXISTS face3 (name TEXT, buffer BLOB, st TEXT)");
    if(!db.open())
    {
...
void MainWindow::savedatabase(QString name, std::vector<uchar> buffer, std::vector<matrix<float,0,1>> face_descriptors)
{
    ...
    QString st = matrixToByteArray(face_descriptors);
    qDebug() << "dataString:" << st;

    //QSqlQuery query; //创建数据库表
    //数据存储在名为"face3"的表中
    query->prepare("INSERT INTO face3 (name, buffer, st) VALUES (?, ?, ?)");
...
/*--------------@brief：将vector<dlib::matrix<float,0,1>>转换为QByteArray--------------*/
QString MainWindow::matrixToByteArray(const std::vector<dlib::matrix<float,0,1>>& face_descriptors)
{   
    QStringList dataStrings;
    for (const auto& matrix : face_descriptors)
    {
        // Convert matrix data to a string
        QStringList matrixData;
        for(float value : matrix)
            matrixData << QString::number(value);
        // Join matrix data with a separator
        QString matrixString = matrixData.join(",");
        dataStrings << matrixString;
    }
