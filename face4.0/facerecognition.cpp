...
void faceRecognition::CalculateMatchingDegree(std::vector<matrix<float,0,1>> faceFeatureInf)
{
    ...
    size_t   minIndex    = -1;  //用于记录最小距离对应的索
    ...
    {       
        ...
        //处理图像数据
        QImage image;
        image.loadFromData(buffer, ".jpg"); //这里需要指定正确的图像格式和编码
        faceImageLibrary.push_back(colorImage);
