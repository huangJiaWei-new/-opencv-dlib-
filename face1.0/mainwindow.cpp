...
m_timer(new QTimer)
{
    ...
    //打印人脸特征信息
    //connect(ui->pushButton_save,&QPushButton::clicked,this,&MainWindow::ExFaceFeatureInformation);

    //connect(ui->read,&QPushButton::clicked,this,&MainWindow::read);
...
{      
    ...
    //cv::equalizeHist(gray, gray);                  //生成灰度图像直方图
    ...
    ExFaceFeatureInformation(faceROI);
    //savedatabase(ui->lineEdit_name->text(), buffer, face_descriptors); //将人脸图像与对应编号存入数据库
}
...
/**
 * @brief 点击identify按钮打开recognize.ui界面
 */
//void MainWindow::on_identify_clicked()
//{
//    recognize *_recognize=new recognize;
//    _recognize->setModal(true);
//    _recognize->exec();
//}
...
void MainWindow::savedatabase(QString name, std::vector<uchar> buffer, std::vector<matrix<float,0,1>> face_descriptors)
{
    ...
    if(!query.exec())
    {
        qDebug() << "Failed to insert face to database:" <<buffer.data()<<buffer.size();
    }
    else
    {
 ...
 * @brief 将检测人脸特征并存入数据库
 */
void MainWindow::ExFaceFeatureInformation(cv::Mat image)
{
    ...
    std::vector<matrix<rgb_pixel>> faces; //声明人脸容器
    ...
    {
        const matrix<float,0,1>& feature = face_descriptors[i];
        std::vector<uchar> byteArray = matrixToByteArray(feature);

        QSqlQuery query; //创建数据库表
        // 将字节数组存入数据库
        query.prepare("INSERT INTO face_data (eature) VALUES (:feature)");
        //query.bindValue(":id", i);
        //query.prepare("INSERT INTO face3 (name, image, feature) VALUES (:name, :image, :feature)");
        query.bindValue(":feature", QVariant::fromValue(byteArray));
        if(!query.exec())
        {
            qDebug() << "Failed to insert descriptors to database:";
        }
        else
        {
            qDebug() << "Face inserted descriptors successfully!";
        }
    }*/
    ...
    for(auto face : detector(img))
    {
        auto shape = sp(img, face);  //对人脸"face"进行关键点定位
        matrix<rgb_pixel> face_chip; //存储提取的人脸图像
        //dlib::array2d<unsigned char> face_chip;//存储提取的人脸图像
